...
namespace UnboundMechanic
{
    public class BoundNPCConfig : ModConfig { // Vanilla bound NPCs
        ...
        public bool None {
            ...
            set {
                if (value) {
                    ...
                    TruffleDoesntRequireSurfaceMushroom = false;
                    TruffleAvailablePreHardmode = false;
                }
            }
        }
        ...
        [Header("Truffle")]
        public bool TruffleDoesntRequireSurfaceMushroom { get; set; }
        public bool TruffleAvailablePreHardmode { get; set; }

        #endregion
    }
    
    public class RestrictionlessSpawningConfig : ModConfig {
        // TODO: Revamp this class to be for restrictionless NPC spawning
        public override ConfigScope Mode => ConfigScope.ServerSide;

        // Attempt to update the list when the config changes
        public override void OnChanged() {
            if (!Main.gameMenu) { // In a world
                ModContent.GetInstance<TownNPCSpawnSystem>()?.UpdateSpawnableTownNPCs(
                    ModContent.GetInstance<RestrictionlessSpawningConfig>()._everyoneCanSpawnFromStart);
                Main.NewText("Trying to update townNPC list");
            }
        ...
        public bool AllowMultipleTravellingMerchantSpawns { get; set; }
        ...
        public List<NPCDefinition> NPCList = [
            // Prehardmode
